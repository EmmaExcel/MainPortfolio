...
export const Works = () => {
  const fullstackProjects = data.filter(p => p.category === "fullstack");
  const frontendProjects = data.filter(p => p.category === "frontend");
  ...
  return (
    <>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-gutter mb-32">
        {fullstackProjects.map((work, index) => renderCard(work, index))}
      </section>

      <section id="design-works" className="flex justify-between items-end mb-12 border-b border-white/10 pb-4 fade-in-up">
        <h3 className="font-headline-lg text-headline-lg-mobile md:text-headline-lg text-primary uppercase">Design & Frontend</h3>
        <span className="font-headline-md text-headline-md text-on-surface-variant">(2021-2023)</span>
      </section>

      <section className="grid grid-cols-1 md:grid-cols-2 gap-gutter mb-32">
        {frontendProjects.map((work, index) => renderCard(work, index))}
      </section>
    </>
  );
};